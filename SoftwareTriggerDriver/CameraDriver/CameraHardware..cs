...
namespace ASCOM.RobertDo3Think_USB3_12M_Camera_Trigger.Camera
{
    ...
    internal static class CameraHardware
    {
        ...
        private static string DriverDescription = "RobertDo3Think USB3 12M Software Trigger Driver"; // The value is set by the driver's class initialiser.
        ...
        internal const string driverversion = "0.2-trigger";
        ...
        static CameraHardware()
        {
            try
            {
                ...
                tl = new TraceLogger("", "RobertDo3Think_USB3_12M_Camera_Trigger.Hardware");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating software-trigger camera driver", MessageBoxButtons.OK, MessageBoxIcon.Error);
                throw;
            }
        }
                ...
                if (value)
                {
                    ...
                    try
                    {
                        ...
                        lock (cameraLock)
                        {
                            ...
                            StartSoftwareTriggerStream();
                            captureSignature = BuildCaptureSignature();
                            lastTriggeredFrameId = 0;
        ...
        public static string Name
        {
            ...
            get
            {
                string name = "U3s1201 Software Trigger";
        ...
        static private bool fastRestartProbePending = false;
        static private bool triggerRequestPending = false;
        static private long triggerRequestSequence = 0;
        static private bool requestedLightFrame = true;
        static private ulong lastTriggeredFrameId = 0;
        ...
        static internal void StartExposure(double Duration, bool Light)
        {
            ...
            ValidateSubframe();
            EnsureTriggerCaptureInfrastructure(Duration);

            lock (cameraLock)
            {
                if (cameraState == CameraStates.cameraExposing || triggerRequestPending)
                {
                    throw new ASCOM.InvalidOperationException("StartExposure was called while a software-triggered exposure is already active.");
                }
                ...
                requestedLightFrame = Light;
                captureFailureMessage = string.Empty;
                ResetCapturedFrame();
                triggerRequestSequence++;
                triggerRequestPending = true;
                LogMessage("StartExposure", $"Queued software trigger request {triggerRequestSequence}: duration {Duration}s, Light {Light}.");
                System.Threading.Monitor.PulseAll(cameraLock);
            }
        }
        /// <summary>
        ...
        private static void EnsureTriggerCaptureInfrastructure(double duration)
        {
            bool captureWorkerAlive;
            lock (cameraLock)
            {
            ...
            if (captureWorkerAlive) return;

            LogMessage("StartExposure", "Software-trigger capture worker is not running; rebuilding trigger infrastructure.");
            StopStreamIfStarted();
            try
            {
        ...
        private static void StartSoftwareTriggerStream()
        {
            LogOptionalStatus(DVPCamera.dvpSetSoftTriggerLoopState(CameraHardware.handle, false), "dvpSetSoftTriggerLoopState(false)");
            CheckStatus(DVPCamera.dvpSetTriggerSource(CameraHardware.handle, dvpTriggerSource.TRIGGER_SOURCE_SOFTWARE), "dvpSetTriggerSource(SOFTWARE)");
            CheckStatus(DVPCamera.dvpSetFramesPerTrigger(CameraHardware.handle, 1), "dvpSetFramesPerTrigger(1)");
            CheckStatus(DVPCamera.dvpSetTriggerState(CameraHardware.handle, true), "dvpSetTriggerState(true)");
            ConfigureFrameQueue();
            ...
            {
                LogMessage("dvpStart", $"Accepted software-trigger stream status {startStatus}.");
            }
            captureStartupTimeoutPending = false;
            fastRestartProbePending = false;
            LogMessage("Trigger", "Software trigger mode armed: source SOFTWARE, one output frame per trigger.");
        }
        ...
        private static void StartCaptureThread()
        {
            ...
            captureThread.Name = "DVP software-trigger capture";
            captureThread.Start();
        }
        ...
        private static void CaptureLoop()
        {
            try
            {
                while (!captureThreadStop)
                {
                    ...
                    lock (cameraLock)
                    {
                        while (!captureThreadStop && !triggerRequestPending)
                        {
                            System.Threading.Monitor.Wait(cameraLock);
                        }
                        if (captureThreadStop) break;

                        duration = requestedCaptureDuration;
                        light = requestedLightFrame;
                        requestSequence = triggerRequestSequence;
                        triggerRequestPending = false;
                    }

                    CaptureSoftwareTriggeredRequest(requestSequence, duration, light);
                }
            }
            catch (Exception ex)
            {
        ...
        private static void CaptureSoftwareTriggeredRequest(long requestSequence, double duration, bool light)
        {
            dvpStatus lastStatus;
            string lastError;
            if (TryCaptureSoftwareTriggeredFrame(requestSequence, duration, light, out lastStatus, out lastError))
            {
                return;
            }
            if (captureThreadStop) return;

            LogMessage("Trigger", $"Request {requestSequence} failed with {lastStatus}: {lastError}. Attempting one controlled trigger-stream restart.");
            if (TryRecoverStreamOnCaptureThread() &&
                TryCaptureSoftwareTriggeredFrame(requestSequence, duration, light, out lastStatus, out lastError))
            {
                LogMessage("Trigger", $"Request {requestSequence} recovered after trigger-stream restart.");
                return;
            }

            for (int reopenAttempt = 1; reopenAttempt <= MaximumFullReopenAttempts && !captureThreadStop; reopenAttempt++)
            {
                LogMessage("Trigger", $"Request {requestSequence}: full camera reopen attempt {reopenAttempt}/{MaximumFullReopenAttempts}.");
                if (TryReopenCameraOnCaptureThread() &&
                    TryCaptureSoftwareTriggeredFrame(requestSequence, duration, light, out lastStatus, out lastError))
                {
                    LogMessage("Trigger", $"Request {requestSequence} recovered after full camera reopen {reopenAttempt}.");
                    return;
                }

                if (reopenAttempt < MaximumFullReopenAttempts && !captureThreadStop)
                {
                    System.Threading.Thread.Sleep(RecoveryRetryDelayMilliseconds);
                }
            }
            ...
            if (captureThreadStop) return;
            throw new DriverException($"Software-trigger request {requestSequence} produced no valid frame. Last SDK status: {lastStatus}. {lastError}");
        }

        private static bool TryCaptureSoftwareTriggeredFrame(long requestSequence, double duration, bool light, out dvpStatus lastStatus, out string error)
        {
            lastStatus = dvpStatus.DVP_STATUS_FAILED;
            error = string.Empty;
            try
            {
                PrepareSoftwareTriggeredRequest(duration);
                DrainQueuedTriggerFrames();
                if (captureThreadStop) return false;

                DateTime triggerTime = DateTime.UtcNow;
                dvpStatus triggerStatus = DVPCamera.dvpTriggerFire(CameraHardware.handle);
                if (triggerStatus != dvpStatus.DVP_STATUS_OK && triggerStatus != dvpStatus.DVP_STATUS_IN_PROCESS)
                {
                    lastStatus = triggerStatus;
                    error = "dvpTriggerFire did not accept the software trigger.";
                    return false;
                }
                LogMessage("Trigger", $"Request {requestSequence}: dvpTriggerFire accepted {triggerStatus}; waiting for one frame.");

                uint timeout = CalculateTriggeredFrameTimeout(appliedCaptureDuration);
                for (int frameAttempt = 1; frameAttempt <= 2 && !captureThreadStop; frameAttempt++)
                {
                    dvpFrame frame = new dvpFrame();
                    IntPtr rawPtr = IntPtr.Zero;
                    lastStatus = DVPCamera.dvpGetFrame(CameraHardware.handle, ref frame, ref rawPtr, timeout);
                    if (lastStatus != dvpStatus.DVP_STATUS_OK)
                    {
                        error = $"dvpGetFrame timed out or failed after software trigger (timeout {timeout}ms).";
                        return false;
                    }

                    ValidateFrame(frame, rawPtr);
                    if (frame.uFrameID != 0 && lastTriggeredFrameId != 0 && frame.uFrameID <= lastTriggeredFrameId)
                    {
                        LogMessage("Trigger", $"Request {requestSequence}: discarded stale frame ID {frame.uFrameID}, previous ID {lastTriggeredFrameId}.");
                        continue;
                    }

                    double actualDuration = frame.fExposure > 0 ? frame.fExposure / 1000000.0 : appliedCaptureDuration;
                    if (actualDuration > 0 && appliedCaptureDuration > 0 && actualDuration < appliedCaptureDuration * 0.8)
                    {
                        if (frame.uFrameID != 0) lastTriggeredFrameId = frame.uFrameID;
                        LogMessage("Trigger", $"Request {requestSequence}: discarded transitional frame ID {frame.uFrameID}, exposure {actualDuration}s instead of {appliedCaptureDuration}s.");
                        continue;
                    }
                    ...
                    lock (cameraLock)
                    {
                        if (captureThreadStop) return false;
                        if (frame.uFrameID != 0) lastTriggeredFrameId = frame.uFrameID;
                        StoreSoftwareTriggeredFrame(frame, rawPtr, actualDuration, triggerTime, light);
                        captureFailureMessage = string.Empty;
                        System.Threading.Monitor.PulseAll(cameraLock);
                    }
                    LogMessage("Trigger", $"Request {requestSequence}: accepted frame ID {frame.uFrameID}, actual exposure {actualDuration}s.");
                    return true;
                }

                lastStatus = dvpStatus.DVP_STATUS_TIME_OUT;
                error = "Only stale or transitional frames were returned for this software trigger.";
                return false;
            }
            catch (Exception ex)
            {
                lastStatus = dvpStatus.DVP_STATUS_FAILED;
                error = ex.Message;
                LogMessage("Trigger", $"Request {requestSequence} capture attempt threw: {ex}");
                return false;
            }
        }

        private static void PrepareSoftwareTriggeredRequest(double duration)
        {
            string desiredSignature = BuildCaptureSignature();
            if (!string.Equals(captureSignature, desiredSignature, StringComparison.Ordinal))
            {
                dvpStatus stopStatus = DVPCamera.dvpStop(CameraHardware.handle);
                if (stopStatus != dvpStatus.DVP_STATUS_OK &&
                    stopStatus != dvpStatus.DVP_STATUS_NOT_STARTED &&
                    stopStatus != dvpStatus.DVP_STATUS_DEVICE_IS_STOPPED)
                {
                    throw new DriverException($"dvpStop before triggered ROI change failed: {stopStatus}");
                }
                System.Threading.Thread.Sleep(100);
                ConfigureManualExposure(duration);
                ConfigureAnalogGain(requestedGain);
                StartSoftwareTriggerStream();
                captureSignature = desiredSignature;
            }
            else
            {
                if (!ExposureDurationsMatch(configuredCaptureRequestDuration, duration))
                {
                    ConfigureExposureOnly(duration);
                }
                if (appliedGain != requestedGain)
                {
                    ConfigureAnalogGain(requestedGain);
                }
            }
            ...
            LogMessage("Trigger", $"Prepared one-shot request: requested {duration}s, actual {appliedCaptureDuration}s, gain {appliedGain}, ROI {desiredSignature}.");
        }

        private static void DrainQueuedTriggerFrames()
        {
            int drained = 0;
            for (int index = 0; index < SdkFrameQueueSize + 2; index++)
            {
                dvpFrame frame = new dvpFrame();
                IntPtr rawPtr = IntPtr.Zero;
                dvpStatus status = DVPCamera.dvpGetFrame(CameraHardware.handle, ref frame, ref rawPtr, 1);
                if (status != dvpStatus.DVP_STATUS_OK) break;
                if (frame.uFrameID != 0) lastTriggeredFrameId = frame.uFrameID;
                drained++;
            }
            if (drained > 0)
            {
                LogMessage("Trigger", $"Drained {drained} queued frame(s) before issuing a new software trigger.");
            }
        }

        private static void StoreSoftwareTriggeredFrame(dvpFrame frame, IntPtr rawPtr, double duration, DateTime triggerTime, bool light)
        {
            EnsureImageArray();
            CopyFrameToImageArray(frame, rawPtr, captureImageArray, duration);
            capturedFrameAvailable = true;
            capturedFrameDuration = duration;
            capturedFrameStartTime = triggerTime;
            capturedFrameArrivalTime = DateTime.UtcNow;
            capturedFrameGain = appliedGain;
            capturedFrameVersion++;
            PublishCapturedFrameLocked(requestedCaptureDuration, light);
        }
        ...
        private static bool TryRecoverStreamOnCaptureThread()
        {
            try
            {
                bool online = false;
                dvpStatus onlineStatus = DVPCamera.dvpIsOnline(CameraHardware.handle, ref online);
                if (onlineStatus != dvpStatus.DVP_STATUS_OK || !online)
                {
                    LogMessage("Capture", $"Stream recovery skipped because camera online check returned {onlineStatus}, online={online}.");
                    return false;
                }

                // A timed-out trigger is ambiguous: the sensor might still finish it later.
                // Stop the stream first so retrying cannot create two output frames for one ASCOM
                // request, then restore software-trigger mode without replacing the native handle.
                LogMessage("Trigger", "Cancelling uncertain trigger and re-arming the software-trigger stream.");
                dvpStatus stopStatus = DVPCamera.dvpStop(CameraHardware.handle);
                if (stopStatus != dvpStatus.DVP_STATUS_OK &&
                    stopStatus != dvpStatus.DVP_STATUS_NOT_STARTED &&
                    stopStatus != dvpStatus.DVP_STATUS_DEVICE_IS_STOPPED)
                {
                    LogMessage("Trigger", $"Controlled trigger recovery dvpStop returned {stopStatus}; escalating to device reopen.");
                    return false;
                }
                ...
                LogMessage("Trigger", "Controlled software-trigger stream restart completed.");
                return true;
            }
            catch (Exception ex)
            {
                LogMessage("Trigger", $"Controlled software-trigger recovery failed: {ex.Message}");
                return false;
            }
        }

        private static bool IsFastStreamRecoveryAccepted(dvpStatus status)
        {
            // NOT_STOPPED is tolerated by dvpStart because it can mean an already-running stream,
            // but it does not prove that dvpRestart performed a recovery.
            return status == dvpStatus.DVP_STATUS_OK ||
                   status == dvpStatus.DVP_STATUS_IN_PROCESS ||
                   status == dvpStatus.DVP_STATUS_DEVICE_IS_STARTED;
        }
        ...
        private static bool TryReopenCameraOnCaptureThread()
        {
            ...
            try
            {
                ...
                LogMessage("Trigger", "Full camera close/open cycle completed and software-trigger mode was re-armed.");
                return true;
            }
            catch (Exception ex)
            {
        ...
        private static dvpStatus CaptureNextFrame()
        {
            ...
            bool startupTimeout;
            bool fastRestartProbe;
            lock (cameraLock)
            {
                activeDuration = appliedCaptureDuration;
                startupTimeout = captureStartupTimeoutPending;
                fastRestartProbe = fastRestartProbePending;
            }
            uint timeout = startupTimeout && !fastRestartProbe
                ? CalculateStartupFrameTimeout(activeDuration)
                : CalculateFrameTimeout(activeDuration);

            // Use one uninterrupted SDK wait. Splitting a long wait into two dvpGetFrame calls
            // caused false timeouts because the first valid frame after an exposure change or
            // stream restart can require roughly two exposure periods. A second call is made only
            // when the SDK actually returned a transitional frame whose exposure is too short.
            for (int frameAttempt = 1; frameAttempt <= 2 && !captureThreadStop; frameAttempt++)
            {
        ...
        private static uint CalculateTriggeredFrameTimeout(double duration)
        {
            // A software trigger starts exactly one exposure, so only one exposure period plus
            // USB/readout margin is required. This deliberately avoids the two-exposure wait
            // that was necessary when selecting a matching frame from a continuously running stream.
            double timeout = Math.Ceiling(duration * 1000.0) + FrameReadoutMarginMilliseconds;
            if (timeout < MinimumFrameTimeoutMilliseconds) timeout = MinimumFrameTimeoutMilliseconds;
            if (timeout > uint.MaxValue) timeout = uint.MaxValue;
            return (uint)timeout;
        }
        ...
        {
            double timeout = Math.Ceiling(duration * 2000.0) + FrameReadoutMarginMilliseconds;
            // Logs show that a newly reopened USB stream can need between ten and twenty seconds
            // for its first short-exposure frame. This replaces two separate ten-second waits
            // with one uninterrupted twenty-second wait.
        ...
        private static void StopStreamIfStarted()
        {
            ...
            lock (cameraLock)
            {
                ...
                // The software-trigger worker normally sleeps on this monitor while idle.
                // Wake it before joining so disconnect and recovery cannot wait for a request
